...
cv::Mat project(cv::Mat projection_matrix, cv::Rect frame, pcl::PointCloud<pcl::PointXYZ> point_cloud, pcl::PointCloud<pcl::PointXYZ> *visible_points)
{
	...
	if (xy.inside(frame))
	{
		...
		plane.at<float>(xy)=250; //投影到image处的pixel灰度值为250，其他元素为0
	}
	}

	cv::Mat plane_gray;
	cv::normalize(plane, plane_gray, 0, 255, cv::NORM_MINMAX, CV_8UC1); //[alpha, beta] = [0, 255]
	cv::dilate(plane_gray, plane_gray, cv::Mat()); //进行膨胀操作
