...
{
    //作者默认是16线velodyne
	std::vector<std::vector<myPointXYZRID*>> rings(16); //每个点的地址 
	
	for(pcl::PointCloud<myPointXYZRID>::iterator pt = point_cloud.points.begin() ; pt < point_cloud.points.end(); pt++){
		pt->range = (pt->x * pt->x + pt->y * pt->y + pt->z * pt->z); //距离需要自己额外计算
		rings[pt->ring].push_back(&(*pt));  //point_cloud中每个点的地址同时保存在rings[][]中
	}
	...
	for(std::vector<std::vector<myPointXYZRID*>>::iterator ring = rings.begin(); ring < rings.end(); ring++){
		...
		for (std::vector<myPointXYZRID*>::iterator pt = ring->begin() + 1; pt < ring->end() - 1; pt++)
		{
            ...
            //根据与左右相邻两个点的距离差的max，作为该点的intensity
			(*pt)->intensity = MAX( MAX( prev->range-(*pt)->range, succ->range-(*pt)->range), 0) * 10; 
		}
	}
	point_cloud = normalizeIntensity(point_cloud, 0.0, 1.0); //归一化
	...
	//pcl::io::savePCDFileASCII ("/home/vishnu/lidar_camera_calib_filtered_in_rgb_optical_frame.pcd", *(toPointsXYZ(filtered)));
	return filtered;
}
